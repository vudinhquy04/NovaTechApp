...
const OrderHistoryScreen = ({ navigation, route }) => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      console.log('Orders length:', Array.isArray(allOrders) ? allOrders.length : 'Not array');
      console.log('========================');
  ...
  const renderOrderCard = (order) => {
    ...
    const isDelivering = status === 'shipped' || status === 'delivering';
    const isDelivered = status === 'delivered';
    const isCancelled = status === 'cancelled';

    console.log('=== ORDER CARD DEBUG ===');
            ...
            <>
              ...
              <View style={styles.productInfo}>
                <Text style={styles.productName}>
                  Sản phẩm: {order.items[0].name || order.items[0].product?.name || 'Sản phẩm'} (x{order.items[0].quantity || 1})
                </Text>
